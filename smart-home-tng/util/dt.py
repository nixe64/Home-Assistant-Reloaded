"""
Helper methods to handle the time in Smart Home - The Next Generation.
...
def set_default_time_zone(time_zone: dt.tzinfo) -> None:
    ...
    Async friendly.
    """
    global _default_time_zone  # pylint: disable=global-statement
...
def now(time_zone: dt.tzinfo | None = None) -> dt.datetime:
    ...
    return dt.datetime.now(time_zone or _default_time_zone)
...
def as_utc(dattim: dt.datetime) -> dt.datetime:
    ...
    if dattim.tzinfo is None:
        dattim = dattim.replace(tzinfo=_default_time_zone)
...
def as_local(dattim: dt.datetime) -> dt.datetime:
    ...
    if dattim.tzinfo == _default_time_zone:
        return dattim
    if dattim.tzinfo is None:
        dattim = dattim.replace(tzinfo=_default_time_zone)

    return dattim.astimezone(_default_time_zone)


def utc_from_timestamp(timestamp: float) -> dt.datetime:
    """Return a UTC time from a timestamp."""
    return dt.datetime.utcfromtimestamp(timestamp).replace(tzinfo=_UTC)
...
def start_of_local_day(dt_or_d: dt.date | dt.datetime | None = None) -> dt.datetime:
    ...
    return dt.datetime.combine(date, dt.time(), tzinfo=_default_time_zone)
...
def parse_date(dt_str: str) -> dt.date | None:
    ...
    try:
        return dt.datetime.strptime(dt_str, _DATE_STR_FORMAT).date()
...
def parse_time_expression(parameter: typing.Any, min_value: int, max_value: int) -> list[int]:
