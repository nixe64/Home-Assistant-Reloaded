...
export class HaConfigEntities extends SubscribeMixin(LitElement) {
  ...
  public constructor() {
    ...
    window.addEventListener("location-changed", () => {
      if (
        window.location.search.substring(1) !== this._searchParams.toString()
      ) {
        this._searchPaarms = new URLSearchParams(window.location.search);
      }
    });
  ...
  private _removeSelected() {
    const removableEntities = this._selectedEntities.filter((entity) => {
    ...
    showConfirmationDialog(this, {
      title: this.hass.localize(
        `ui.panel.config.entities.picker.remove_selected.confirm_${
          removableEntities.length !== this._selectedEntities.length
            ? "partly_"
            : ""
        }title`,
        "number",
        removableEntities.length
      ),
      text:
        removableEntities.length === this._selectedEntities.length
          ? this.hass.localize(
  ...
  private _clearFilter() {
    if (
      this._activeFilters(this._searchParams, this.hass.localize, this._entries)
    ) {
