...
export const generateViewConfig = (
  ...
  const ungroupedEntities: { [domain: string]: string[] } = {};
  ...
  Object.keys(splitted.ungrouped).forEach((entityId) => {
    ...
    if (!(domain in ungroupedEntities)) {
      ungroupedEntities[domain] = [];
    }

    ungroupedEntities[domain].push(state.entity_id);
  });

  const cards: LovelaceCardConfig[] = [];

  if ("person" in ungroupedEntities) {
    const personCards: LovelaceCardConfig[] = [];

    if (ungroupedEntities.person.length === 1) {
      cards.push({
        type: "entities",
        entities: ungroupedEntities.person,
      });
    } else {
      let backgroundColor: string | undefined;
      let foregroundColor = "";

      for (const personEntityId of ungroupedEntities.person) {
    ...
    delete ungroupedEntities.person;
  }
