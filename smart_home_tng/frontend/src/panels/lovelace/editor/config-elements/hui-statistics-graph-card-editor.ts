  ...
  implements LovelaceCardEditor
{
  ...
  @state() private _statisticsMetaData?: StatisticsMetaData[];
  ...
  private _getStatisticsMetaData = async (statisticIds?: string[]) => {
    this._statisticsMetaData = await getStatisticMetadata(
      this.hass!,
      statisticIds || []
    );
  };
  ...
  public willUpdate(changedProps: PropertyValues) {
    if (
      ...
      this._statisticsMetaData = undefined;
  ...
  private _schema = memoizeOne(
    (
      ...
      statisticsMetaData?.forEach((metaData) => {
              ...
              selector: {
                select: {
                  ...
                  options: stat_types.map((stat_type) => ({
                    ...
                    disabled:
                      !statisticsMetaData ||
                      !statisticsMetaData.every((metaData) =>
  ...
  protected render(): TemplateResult {
    ...
    const configured_stat_types = this._config!.stat_types
      ...
      : stat_types.filter((stat_type) =>
          this._statisticsMetaData?.every((metaData) =>
            statisticsMetaHasType(metaData, statTypeMap[stat_type])
          )
        );
    const data = {
    ...
    const unitClass = this._statisticsMetaData?.[0]?.unit_class;
    const statisticsUnit = unitClass
      ? undefined
      : this._statisticsMetaData?.[0]?.statistics_unit_of_measurement;
