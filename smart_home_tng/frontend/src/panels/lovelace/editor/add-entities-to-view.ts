...
export const addEntitiesToLovelaceView = async (
  ...
  const storageDashes = dashboards.filter(
  ...
  if (mainLovelaceMode !== "storage" && !storageDashes.length) {
  ...
  if (!lovelaceConfig && storageDashes.length) {
    // find first dashoard not in generated mode
    for (const storageDash of storageDashes) {
      try {
  ...
  if (!lovelaceConfig) {
    if (dashboards.length > storageDashes.length) {
  ...
  if (!storageDashes.length && !lovelaceConfig.views?.length) {
    showAlertDialog(element, {
  ...
  if (!storageDashes.length && lovelaceConfig.views.length === 1) {
    showSuggestCardDialog(element, {
      cardTitle,
