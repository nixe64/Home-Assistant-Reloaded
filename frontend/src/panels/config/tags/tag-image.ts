...
export class HaTagImage extends LitElement {
  ...
  protected updated() {
    const msSinceLastScanned = this.tag?.last_scanned_datetime
      ? new Date().getTime() - this.tag.last_scanned_datetime.getTime()
      : undefined;

    if (msSinceLastScanned && msSinceLastScanned < 1000) {
      if (this._timeout) {
    ...
    } else if (!msSinceLastScanned || msSinceLastScanned > 10000) {
      clearTimeout(this._timeout);
      this._timeout = undefined;
      this.classList.remove("just-scanned");
    }
